...
{
    float Ts = 1.0f / F_CONTROL;

    float proporcional = pi->Kp * erro; // Termo Proporcional
    pi->erro_integrado += pi->Ki * erro * Ts;
    float output = pi->Kp * erro + pi->erro_integrado;

    
    //pi->erro_integrado += erro; // Acumulo de erro para Ki
    //float integrativo = pi->Ki * pi->erro_integrado; // Termo Integrativo

    // Saída total do controlador
    //float output = proporcional + integrativo;
